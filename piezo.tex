...
Residual switching &        \SI{3}{\micro\volt}   & \SI{8}{\micro\volt} \\
\hline
\rule{0pt}{2ex}\textbf{total (calculated)} & \SI{64}{\micro\volt} & \SI{82}{\micro\volt}  \\
\hline\hline
\end{tabular*}
}
...
%To convert the current noise into an equivalent voltage noise referred to the output (RTO), we multiply by the transimpedance gain, $G_\pm$, seen from the relevant node.
%For $i_n$ at the inverting node of U2, $G_-$ is simply the impedance $Z_1$ formed by $R_1$ in parallel with $C_1$,
%\begin{align}
%G_- = \frac{R_1}{1+2\pi i f R_1 C_1}
%\end{align}
%where $f$ is the frequency.
%For current noise at the non-inverting node, we find
%\begin{align}
%%G_+ &= Z_\text{LP}\left(1+\frac{Z_1}{R_2~||~Z_\text{mod}}\right)
%G_+ &= Z_\text{LP}\left[1+Z_1\left(\frac{1}{R_2} + \frac{1}{Z_\text{mod}}\right)\right]\,,
%\end{align}
%where $Z_\text{LP} = R_\text{LP}/(1+2\pi i f R_\text{LP} C_\text{LP})$ is the impedance of the switchable low-pass filter (discussed below) as seen from the non-inverting node of U2, and the term in brackets is the non-inverting voltage gain, $G_\text{pos}$.
%With these values, we calculate the total op-amp noise contribution as
%\begin{align}
%e^2_{n,\text{U2}} &= e_n^2 \left|G_\text{pos}\right|^2 + i_n^2\left( \left|G_-\right|^2 + \left|G_+\right|^2\right)
%\end{align}
