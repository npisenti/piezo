...
\begin{figure*}[t]
\includegraphics[width=\textwidth]{fig/Schematic.ai}
...
%Rework this paragraph re: dan's comment; stabilization vs 
%
%The electronics that control the galvanically isolated output of the DRV2700 works in two ways: keep Cout constant, U2 drives Vmg in order to supress the noise; 
%
%emphasize: sensing Vout relative to the true ground, and servoing Vmg to stabilize (remove noise, DC value...) -- explain high level first, then drill down into specifics?
%
%Note that the modulation gain is negative, so the voltage Vmod is inverted before being summed onto the output.
...
%At low frequencies, this amounts to changing the floating ground reference for the flyback regulator, while at high frequencies the capacitor $\text{C}_\text{out}$ provides a low-impedance pathway to $\text{V}_\text{out}$.
%Thus, U2 simultaneously suppresses residual switching noise from the flyback converter and provides a DC-coupled, high-bandwidth modulation of the output.
...
%%% Comment about the total RMS noise from DRV2700, cancellation by op amp
...
% are colored based on their normally distributed statistical probability, and thus give a visual estimation of the RMS width.
